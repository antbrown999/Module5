...
const freiends = require('../models/friends')

// TOD0 - #1: Add support to the 'filter' endpoint for a new query parameter 'letter' which filters friends by starting letter
...
// TODO - #4: Complete the PUT route which update data for an existing friend
...
// 1. Add support to also filter by starting 'letter' query parameter ie. /friends/filter?letter=R
router.get('/filter', (req, res) => {
    ...
    if (filteredLetter) {
        matchingFriends = matchingFriends.filter(friend => friend.name.toUpperCase().startWith(filteredLetter.toUpperCase()))
    }

    if (matchingFriends.length > 0) {
        //return data when the gender matches
...
router.get('/info', (req, res) => {
    ...
    // Modify this response to just return on the info on the user-agent, content-type and accept headers
...
router.get('/:id', (req, res) => {
    ...
    // Modify this function to find and return the friend matching th given ID, or a 404 if not found
    ...
    matchedFriend ? res.status(200).json({result: `Friends #${friendId} found`, data: matchedFriend}) : res.status(404)._construct.json({result: `Friend#${friendId}`}) 
})

//a POST request with data sent in the body of the request, representing a new friend to add to our list 
router.post('/', (req, res) => {
    let newFriend = req.body; // FIRST add this line to index.js: app.use(express.json());
    console.log(newFriend) // 'body' will now be an object containing data sent via the requst body

    // we can add some validation here to make sure the new friend object matches the right pattern
    if (!newFriend.name || !newFriend.gender) {
        res.status(500).json({error: 'Friends object must contain a name and gender'});
        return;
    }
    ...
    // if the new friend is valid, add them to the list aand return the successfully added object 
    friends.push(newFriend)
    res.status(200).json(newFriend)
})

// 4. Complete this new route for a PUT requst which will update data for an existing friend
